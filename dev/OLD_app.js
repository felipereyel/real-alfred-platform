...
async function run(serieCode = 'all'){
    ...
    if(serieCode == 'all'){
        seriesInfo.map(async serieInfo => {
            const {serieInfoCopy, URLs} = await findEps(serieInfo);
            serieInfo = serieInfoCopy;
            allURLs.push({tittle: serieInfo.tittle, URLs});
            return serieInfo;
        });
        ...
        await seriesInfo.map(async serieInfo => {
            if(serieInfo.codename == serieCode){
                const {serieInfoCopy, URLs} = await findEps(serieInfo);
                serieInfo = serieInfoCopy;
                allURLs.push({tittle: serieInfo.tittle, URLs});
            }
